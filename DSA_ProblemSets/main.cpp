  ...
  public:
    ...
    T pop(){
      ...
      if(emptyListFlag){                               // emptyListFlag is set by pop() function in case of
        ...
        cout<<"Value popped: "<<temp<<"\r\n";          // normally, this line executes
      return temp;
    }
};
...
class Queue: public SimpleList<T>{
  public:
    ...
    T pop(){
      T temp = this->SimpleList<T>::removeTop();
      if(emptyListFlag){
        cout<<"ERROR: This list is empty!\r\n";        // Please see notes for pop() above
        emptyListFlag = false;
      }
      else
        cout<<"Value popped: "<<temp<<"\r\n";
...
void pushList(string& name, string& value,list<SimpleList<int> *>& listSLi, list<SimpleList<double> *>& listSLd, list<SimpleList<string> *>& listSLs){
  ...
  cout<<"ERROR: This name does not exist!\r\n"; // If the SL* pointer is NULL every time, then the list does not exist
}
...
int main(){
  ...
  cout.rdbuf(out.rdbuf()); //redirect cout to outfile
  ...
  while(getline(input,line) ){
    cout<<"PROCESSING COMMAND: "<<line<<"\n"; // Every line is always written back out first
